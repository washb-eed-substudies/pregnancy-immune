...
d <- readRDS("~/Downloads/bangladesh-cleaned-master-data.RDS")
...
for(i in Xvars){
  for(j in Yvars){
    ...
    res_adj <- fit_RE_gam(d=subset_data(j), X=i, Y=j,  W=Wset)
...
#Save results
saveRDS(H1_adj_res, here("results/adjusted/H1_adj_res_noari.RDS"))
...
#Save plot data
saveRDS(H1_adj_plot_data, here("figure-data/H1_adj_spline.data_noari.RDS"))
...
for(i in Xvars){
  for(j in Yvars){
    ...
    res_adj <- fit_RE_gam(d=subset_data(j), X=i, Y=j,  W=Wset, forcedW = c("time_of_day_cort_cont"))
    res <- data.frame(X=i, Y=j, fit=I(list(res_adj$fit)), dat=I(list(res_adj$dat)))
...
#Save results
saveRDS(H2_adj_res, here("results/adjusted/H2_adj_res_noari.RDS"))
...
#Save plot data
saveRDS(H2_adj_plot_data, here("figure-data/H2_adj_spline.data_noari.RDS"))
...
#Save results
saveRDS(H3_res, here("results/adjusted/H3_adj_res_noari.RDS"))
...
#Save plot data
saveRDS(H3_plot_data, here("figure-data/H3_adj_spline.data_noari.RDS"))
...
#Save results
saveRDS(H4_res, here("results/adjusted/H4_adj_res_noari.RDS"))
...
#Save plot data
saveRDS(H4_plot_data, here("figure-data/H4_adj_spline.data_noari.RDS"))
