...
d <- readRDS("~/Downloads/bangladesh-cleaned-master-data.RDS") %>% filter(tr %in% c("Control", "Nutrition + WSH"))
...
n_med_col <- NULL
for (var in c(child)) {
  if (var %in% c('sex', 'diar7d_t2', 'diar7d_t3', 'ari7d_t2', 'ari7d_t3','life_viol_any_t3') | is.factor(exc[[var]])) {
    if (var == 'sex') {
